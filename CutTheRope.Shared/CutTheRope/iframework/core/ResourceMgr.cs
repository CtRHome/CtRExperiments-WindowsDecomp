...
internal class ResourceMgr : NSObject
{
	...
	public NSString loadStringsInfo(int key)
	{
		...
		if (xMLNode != null)
		{
			...
			XMLNode xMLNode2 = xMLNode.findChildWithTagNameRecursively(tag, recursively: false);
	...
	public virtual FontGeneric loadVariableFontInfo(string path, int resID, bool isWvga)
	{
		...
		XMLNode xMLNode2 = xMLNode.findChildWithTagNameRecursively("chars", recursively: false);
		XMLNode xMLNode3 = xMLNode.findChildWithTagNameRecursively("kerning", recursively: false);
	...
	public virtual Texture2D loadTextureImageInfo(string path, XMLNode i, bool isWvga, float scaleX, float scaleY)
	{
		...
		string path2 = fullPathFromRelativePath(path);
		if (flag)
		{
		...
		Texture2D texture2D = new Texture2D().initWithPath(path2, assets: true);
	...
	public virtual void setTextureInfo(Texture2D t, XMLNode i, bool isWvga, float scaleX, float scaleY)
	{
		...
		XMLNode xMLNode = i.findChildWithTagNameRecursively("quads", recursively: false);
		if (xMLNode != null)
		{
		...
		XMLNode xMLNode2 = i.findChildWithTagNameRecursively("offsets", recursively: false);
		if (xMLNode2 == null)
		{
			return;
		}
		...
		XMLNode xMLNode3 = i.findChildWithTagNameRecursively(NSObject.NSS("preCutWidth"), recursively: false);
		XMLNode xMLNode4 = i.findChildWithTagNameRecursively(NSObject.NSS("preCutHeight"), recursively: false);
	...
	{
		return ResDataPhoneFullExperiments.ContentFolder + relPath;
	}
	...
	private void loadResource(int resId)
	{
		...
		else if (resId == 197 || resId == 198 || resId == 199)
		{
			Application.sharedSoundMgr().LoadMusic(resId);
		}
		else if (ResDataPhoneFullExperiments.isSound(resId))
		{
			Application.sharedSoundMgr().getSound(resId);
		}
		else if (ResDataPhoneFullExperiments.isFont(resId))
		{
	...
	public virtual void freeResource(int resId)
	{
		...
		if (ResDataPhoneFullExperiments.isSound(resId))
		{
