...
internal class MathHelper : ResDataPhoneFullExperiments
{
	...
	public static float RND_MINUS1_1 => (float)((double)arc4random() / (double)ARC4RANDOM_MAX * 2.0 - 1.0);

	public static float RND_0_1 => (float)((double)arc4random() / (double)ARC4RANDOM_MAX);
	...
	private static bool overlaps1Way(Vector[] corner, Vector[] other)
	{
		...
		ref Vector reference = ref array[0];
		reference = vectSub(corner[1], corner[0]);
		ref Vector reference2 = ref array[1];
		reference2 = vectSub(corner[3], corner[0]);
		for (int i = 0; i < 2; i++)
		{
			ref Vector reference3 = ref array[i];
			reference3 = vectDiv(array[i], vectLengthsq(array[i]));
	...
	{
		return vect(0f - v.y, v.x);
	}

	public static Vector vectRperp(Vector v)
	{
		return vect(v.y, 0f - v.x);
	}
