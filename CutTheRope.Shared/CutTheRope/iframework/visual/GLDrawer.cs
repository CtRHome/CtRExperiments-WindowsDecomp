...
internal class GLDrawer : NSObject
{
	...
	public static Vector calcPathBezier(Vector[] p, int count, float delta)
	{
		if (count > 2)
		{
			if (!VectorArray.TryGetValue(count - 1, out var value))
			{
				value = new Vector[count - 1];
				VectorArray.Add(count - 1, value);
			}
			for (int i = 0; i < count - 1; i++)
			{
				ref Vector reference = ref value[i];
				reference = calc2PointBezier(p[i].x, p[i].y, p[i + 1].x, p[i + 1].y, delta);
			}
			return calcPathBezier_2(value, count - 1, delta);
		}
		if (count == 2)
		{
			return calc2PointBezier(p[0].x, p[0].y, p[1].x, p[1].y, delta);
		}
	...
	public static Vector calc2PointBezier(float ax, float ay, float bx, float by, float delta)
	{
		float num = 1f - delta;
		Vector result = default(Vector);
		result.x = ax * num + bx * delta;
		result.y = ay * num + by * delta;
		return result;
	}
	...
	public static void drawAntialiasedCurve2(float cx, float cy, float radius, float startAngle, float endAngle, int vertexCount, float width, float fadeWidth, RGBAColor fill)
	{
		...
		array[0] = array2[0];
		array[1] = array2[1];
		ref RGBAColor reference = ref array6[0];
		reference = RGBAColor.transparentRGBA;
		for (int i = 1; i < vertexCount; i += 2)
		{
			...
			ref RGBAColor reference2 = ref array6[6 * i - 5];
			reference2 = RGBAColor.transparentRGBA;
			array6[6 * i - 4] = fill;
			array6[6 * i - 3] = fill;
			array6[6 * i - 2] = fill;
			array6[6 * i - 1] = fill;
			ref RGBAColor reference3 = ref array6[6 * i];
			reference3 = RGBAColor.transparentRGBA;
			ref RGBAColor reference4 = ref array6[6 * i + 1];
			reference4 = RGBAColor.transparentRGBA;
			array6[6 * i + 2] = fill;
			array6[6 * i + 3] = fill;
			array6[6 * i + 4] = fill;
			array6[6 * i + 5] = fill;
			ref RGBAColor reference5 = ref array6[6 * i + 6];
			reference5 = RGBAColor.transparentRGBA;
		}
		array[(vertexCount - 1) * 12 + 2] = array2[vertexCount * 2 - 2];
		array[(vertexCount - 1) * 12 + 3] = array2[vertexCount * 2 - 1];
		ref RGBAColor reference6 = ref array6[(vertexCount - 1) * 6 + 1];
		reference6 = RGBAColor.transparentRGBA;
	...
	public static void drawAntialiasedLine(float x1, float y1, float x2, float y2, float size, RGBAColor color)
	{
		...
		verts[14] = v6.x;
		verts[15] = v6.y;
		ref RGBAColor reference = ref colors[2];
		ref RGBAColor reference2 = ref colors[3];
		ref RGBAColor reference3 = ref colors[4];
		reference = (reference2 = (reference3 = (colors[5] = color)));
