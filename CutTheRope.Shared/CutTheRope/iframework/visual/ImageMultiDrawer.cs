...
internal class ImageMultiDrawer : BaseElement
{
	...
	public void mapTextureQuadAtXYatIndex(int q, float dx, float dy, int n)
	{
		...
		ref Quad2D reference = ref texCoordinates[n];
		reference = image.texture.quads[q];
		ref Quad3D reference2 = ref vertices[n];
		reference2 = Quad3D.MakeQuad3D(dx + image.texture.quadOffsets[q].x, dy + image.texture.quadOffsets[q].y, 0.0, image.texture.quadRects[q].w, image.texture.quadRects[q].h);
	}
