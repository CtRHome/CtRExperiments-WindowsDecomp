...
internal class ScrollableContainer : BaseElement
{
	...
	public float bounceMovementDivide;

	public float maxBounceOffsetX;
	...
	public override bool onTouchMoveXY(float tx, float ty)
	{
		...
		if (!MathHelper.vectEqual(dragStart, impossibleTouch))
		{
			...
			{
				vector2.x /= ((bounceMovementDivide == 0f) ? 2f : bounceMovementDivide);
			}
			if (shouldBounceVertically && ((double)container.y > 0.0 || container.y < (float)(-container.height + height)))
			{
				vector2.y /= ((bounceMovementDivide == 0f) ? 2f : bounceMovementDivide);
			}
	...
	public override bool onTouchUpXY(float tx, float ty)
	{
		...
		bool flag2 = false;
		float num = 15f;
		if (isForAboutMenu && Math.Abs(tx - touch_start_x) < num && Math.Abs(ty - touch_start_y) < num)
		{
			flag2 = true;
		}
		if (((double)touchTimer > 0.0 && ((!movingByInertion && !movingToSpoint) || targetSpoint == spointsNum - 1 || CTRPreferences.isLiteVersion())) || flag2)
		{
			bool flag3 = base.onTouchDownXY(savedTouch.x, savedTouch.y);
			touchReleaseTimer = 0.2f;
			touchTimer = 0f;
			if (dontHandleTouchDownsHandledByChilds && flag3)
			{
				return true;
			}
		}
		...
		if ((double)inertiaTimeoutLeft > 0.0)
		{
			float num2 = inertiaTimeoutLeft / inertiaTimeout;
			move = MathHelper.vectMult(staticMove, (float)((double)num2 * 50.0));
	...
	{
		ref Vector reference = ref spoints[i];
		reference = MathHelper.vect(0f - sx, 0f - sy);
		if (i > spointsNum - 1)
		{
			spointsNum = i + 1;
		}
	...
	public virtual Vector moveContainerBy(Vector off)
	{
		...
		float val = container.y + off.y;
		if (!shouldBounceHorizontally)
		{
			num = (float)Math.Min(Math.Max(-container.width + width, num), 0.0);
		}
		else if (maxBounceOffsetX > 0f)
		{
			num = MathHelper.MIN(MathHelper.MAX(num, (float)(-container.width + width) - maxBounceOffsetX), maxBounceOffsetX);
		}
		if (!shouldBounceVertically)
		{
			val = (float)Math.Min(Math.Max(-container.height + height, val), 0.0);
		}
		Vector result = MathHelper.vectSub(MathHelper.vect(num, val), MathHelper.vect(container.x, container.y));
		container.x = num;
		container.y = val;
		return result;
	}
