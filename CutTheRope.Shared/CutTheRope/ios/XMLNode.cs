...
internal class XMLNode
{
	...
	public NSString this[string key]
	{
		...
		set
		{
			...
			{
				attributes_.Add(key, value.ToString());
			}
			else
			{
				attributes_[key] = value.ToString();
			}
		}
	}
	...
	public XMLNode findChildWithTagNameAndAttributeNameValueRecursively(string tag, string attrName, string attrVal, bool recursively)
	{
		...
		{
			if (item.name == tag && item.attributes() && item.attributes_.TryGetValue(attrName, out var text) && text == attrVal)
			{
				return item;
			}
	...
	private static XMLNode ParseLINQ(string fileName)
	{
		...
		try
		{
			Stream stream = TitleContainer.OpenStream("Content/" + ResDataPhoneFullExperiments.ContentFolder + fileName);
		...
		if (xDocument == null)
		{
			xDocument = XDocument.Parse(ResDataPhoneFullExperiments.GetXml(fileName));
		}
