...
{
	public const int MIN_WINDOW_WIDTH = 500;
	...
	public double WidthAspectRatio => (double)_scaledViewRect.Width / (double)_gameWidth;
	...
	{
		form.MinimumSize = new Size(MIN_WINDOW_WIDTH, ScaledGameHeight(MIN_WINDOW_WIDTH));
	}
	...
	private void UpdateScaledView()
	{
		...
		// CtrRenderer.onSurfaceChanged(_scaledViewRect.Width, _scaledViewRect.Height);
		var viewport = Global.GraphicsDevice.Viewport;
        CtrRenderer.onSurfaceChanged(viewport.Width, viewport.Height);
    }
	...
	public void Save()
	{
		Preferences._setIntforKey(_windowRect.Width, "PREFS_WINDOW_WIDTH", comit: false);
		Preferences._setIntforKey(_windowRect.Height, "PREFS_WINDOW_HEIGHT", comit: false);
		Preferences._setBooleanforKey(_isFullScreen, "PREFS_WINDOW_FULLSCREEN", comit: true);
	}
